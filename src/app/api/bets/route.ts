...
import { createBet, getBetsForUser, getActiveBetForUser } from '@/lib/supabase';
import { z } from 'zod';
...
export async function GET() {
  try {
    ...
    const bets = await getBetsForUser(session.userId);
...
export async function POST(request: Request) {
  try {
    ...
    const activeBet = await getActiveBetForUser(session.userId);
    if (activeBet) {
      return NextResponse.json(
